 ...
 *************************************************************************/
function parseDate(d) { if (!d) return null; return new Date(d); }
    ...
    try {
        ...
        list.forEach(m => {
            ...
            tr.innerHTML = `
                ...
                <td>${escapeHTML(m.supplier_name || '-')}</td> <td>${escapeHTML(m.status || '-')}</td>
...
function renderMachineForm(machine = null) {
    ...
    async function buildForm() {
        ...
        formBody.innerHTML = `
            <div class="form-row"><label>رمز الآلية</label><input id="f_code" value="${escapeHTML(machine?.machine_code || 'HM-')}" required /></div>
    ...
    async function saveMachine() {
        ...
        if (!machineData.machine_code || !machineData.machine_name || !machineData.status) {
            return alert('الرجاء تعبئة الحقول الأساسية (الرمز، الاسم، والحالة).');
        }
...
// ... (بقية الدوال openMachineProfile، renderMachineProfile، renderAddMaintenanceForm، renderAddMaterialForm لم تتغير)

/*************************************************************************
 * نافذة تفاصيل الآلة و تسجيل صيانة (مع خصم المواد)
 *************************************************************************/
...
function renderMachineProfile(machine) {
    ...
    div.innerHTML = `
      ...
      <div style="display:flex;gap:12px;margin-top:12px">
        ...
        <div style="width:260px">
          <button id="addMaintBtn" class="btn">تسجيل صيانة جديدة</button>
          <button id="editThis" class="btn ghost">تعديل الآلة</button>
        </div>
      </div>
      <hr>
      <h3>سجل الصيانة</h3>
    ...
    setTimeout(() => {
        ...
        div.querySelector('#editThis').onclick = () => { closeModal(); openEditMachine(machine.machine_id); };
        div.querySelector('#addMaintBtn').onclick = () => { openModal(renderAddMaintenanceForm(machine)); };
...
// نموذج تسجيل صيانة (يسجل صيانة + يخصم المواد)
function renderAddMaintenanceForm(machine) {
    const div = document.createElement('div');
    // build options for materials checklist
    const materialOptions = materials.map(m => `<div style="display:flex;gap:8px;align-items:center;margin-bottom:6px">
      <input type="checkbox" data-id="${m.item_id}" class="mat-ck" />
      <div style="flex:1">${escapeHTML(m.item_name)} <span class="small-muted">(${Number(m.quantity).toFixed(2)} بالمخزون)</span></div>
      <input type="number" min="0" step="0.01" class="mat-qty" data-id="${m.item_id}" placeholder="الكمية" style="width:90px" />
    </div>`).join('');

    div.innerHTML = `
      <h2>تسجيل صيانة — ${escapeHTML(machine.machine_name)}</h2>
      <div class="form-row"><label>التاريخ</label><input id="maint_date" type="date" value="${new Date().toISOString().slice(0, 10)}" /></div>
      <div class="form-row"><label>نوع الصيانة</label><input id="maint_type" value="صيانة دورية" /></div>
      <div class="form-row"><label>الفني</label><input id="maint_tech" value="" /></div>
      <div class="form-row"><label>ملاحظات</label><textarea id="maint_notes" rows="3"></textarea></div>
      <h4>المواد المستخدمة (اختر ثم أدخل الكمية)</h4>
      <div style="max-height:240px;overflow:auto;padding:8px;border:1px solid #f1f5f9;border-radius:8px">${materialOptions || '<div class="small-muted">لم تُعرّف مواد في المخزون بعد.</div>'}</div>
      <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:8px">
        <button id="saveMaint" class="btn">حفظ الصيانة</button>
        <button id="cancelMaint" class="btn ghost">إلغاء</button>
      </div>`;

    setTimeout(() => {
        div.querySelector('#cancelMaint').onclick = closeModal;
        div.querySelector('#saveMaint').onclick = () => {
            const date = div.querySelector('#maint_date').value;
            const type = div.querySelector('#maint_type').value.trim();
            const tech = div.querySelector('#maint_tech').value.trim();
            const notes = div.querySelector('#maint_notes').value.trim();
            if (!date || !type) return alert('يجب إدخال التاريخ ونوع الصيانة.');

            // إنشاء سجل الصيانة (محلية مؤقتاً)
            const mId = machine.machine_id;
            const newMaint = {
                id: nextId(maintenances),
                machine_id: mId,
                date,
                type,
                technician: tech,
            ...
            save(STORAGE_KEYS.maintenances, maintenances);

            // تحديث آخر صيانة في الآلة (محلية مؤقتاً)
            const lastDate = date;
            machines = machines.map(mm => mm.machine_id === mId ? { ...mm, last_maintenance_date: lastDate } : mm);
            save(STORAGE_KEYS.machines, machines);

            // معالجة المواد: خصم من المخزون وتسجيل الاستخدام (محلية مؤقتاً)
            const checked = Array.from(div.querySelectorAll('.mat-ck')).filter(c => c.checked);
            checked.forEach(ch => {
                const id = Number(ch.getAttribute('data-id'));
                const qtyInput = div.querySelector(`.mat-qty[data-id="${id}"]`);
                const qty = parseFloat(qtyInput?.value || 0);
                if (qty <= 0) return; // لا نخصم صفر
                // إنشاء سجل استخدام
                const useRecord = {
                    id: nextId(usage),
                    maintenance_id: newMaint.id,
                    machine_id: mId,
                    item_id: id,
                    quantity_used: qty,
                    used_at: (new Date()).toISOString(),
                    recorded_by: 'المستخدم التجريبي'
                };
                usage.push(useRecord);
                // خصم من المواد
                materials = materials.map(it => it.item_id === id ? { ...it, quantity: (Number(it.quantity || 0) - qty) } : it);
            });
            save(STORAGE_KEYS.usage, usage);
            save(STORAGE_KEYS.materials, materials);

            alert('تم تسجيل الصيانة وتحديث المخزون (إن وُجدت مواد مستخدمة).');
            closeModal();
            renderTable();
        };
    }, 0);
